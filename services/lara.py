...
class Intents:
	...
	def _generate(self, item):
		...
		if 'typo_stem' not in item:
			if item['wordclass'] in ('regex', 'emoji'):
				...
				item['typo_stem'] = trim(strip_accents(remove_double_letters(item['stem'])))
				if not item['typo_stem']:
					item['typo_stem'] = trim(remove_double_letters(item['stem']))
			...
			if 'typo_prefix' not in item:
				if isinstance(item['prefix'], list):
					typo_prefix = ['(?:' + self._scramble(
						trim(strip_accents(remove_double_letters(elem))),
						(item['wordclass'] == 'adjective')) + ')' for elem in item['prefix']]
					item['typo_prefix'] = r'(?:' + ('|'.join(typo_prefix)) + r')?\s?'
				else:
					item['typo_prefix'] = r'' + trim(
						strip_accents(remove_double_letters(item['prefix'])))
			else:
		...
		if 'affix' not in item or not item['affix']:
			...
			if 'typo_affix' not in item:
				if isinstance(item['affix'], list):
					...
					item['typo_affix'] = r'' + strip_accents(item['affix'])
			else:
	...
	def _scramble(self, text, is_adjective=False):
		if len(text) > 3:
			...
			is_consonant = is_consonant(text[-1])
	...
	def _get_clean_text(self, text, deep):
		text = trim(text)
		typo_text = strip_accents(remove_double_letters(text))
		c_text = text
		...
		if text:
			...
			c_text = trim(c_text).split()
			c_typo_text = trim(c_typo_text).split()
			fix_text = []
			last = 0
			for word in c_text:
				x_word = strip_accents(remove_double_letters(word))
	...
	def _get_score(self, text, greedy=True):
		...
		score = {}
		if text:
			typo_text = trim(strip_accents(remove_double_letters(text.replace(',', ' '))))
			for key, value in self.intents.items():
				for item in self.intents[key]:
					found = False
...
class Extract:
	...
	def digits(self, n=0, normalize=True, convert=True):
		...
		if self.text:
			...
			for item in matches:
				original = item
				item = trim(''.join(e for e in item if e.isdigit()))
			...
			if normalize:
				...
				for _item in matches:
					...
					if len(item.strip()) > 2:
						...
						elif (del_matches and 'előtt' not in item
            and 'után' not in item):
							if 'jf' in item:
...
class _re:
	...
	def words(text):
		...
		if t_hash not in _re.words_cache:
			_re.words_cache[t_hash] = number_of_words(text)
